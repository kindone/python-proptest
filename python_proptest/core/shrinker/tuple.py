...
def shrink_tuple(shrinkables: List[Shrinkable[T]]) -> Shrinkable[Tuple[T, ...]]:
    ...
    for position in range(len(shrinkables)):
        def make_shrink_at_position(pos: int):
            ...
            def shrink_at_position(parent: Shrinkable[List[Shrinkable[T]]]) -> Stream[Shrinkable[List[Shrinkable[T]]]]:
                """
                ...
                def create_tuple_with_shrunk_element(shrunken_elem_val: T) -> List[Shrinkable[T]]:
                ...
                tuple_with_shrunk_elem = elem_shrinkable.map(create_tuple_with_shrunk_element)
    ...
    return tuple_shrinkable.map(lambda shrinkable_list: tuple(shr.value for shr in shrinkable_list))
