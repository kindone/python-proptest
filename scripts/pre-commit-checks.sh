...
main() {
    ...
    if ! run_check "Critical flake8 checks" "flake8 python_proptest --count --select=E9,F63,F7,F82 --show-source --statistics"; then
        failed_checks+=("Critical flake8")
    fi

    # Step 3: Extended flake8 checks
    if ! run_check "Extended flake8 checks" "flake8 python_proptest --count --exit-zero --max-complexity=10 --max-line-length=88 --statistics"; then
        failed_checks+=("Extended flake8")
    fi

    # Step 4: Black formatting check
    if ! run_check "Black formatting check" "black --check python_proptest/ tests/"; then
    ...
    if ! run_check "Import sorting check" "isort --check-only python_proptest/ tests/"; then
    ...
    if ! run_check "MyPy type checking" "mypy python_proptest/"; then
    ...
    if ! run_check "Pytest tests with coverage" "pytest --cov=python_proptest --cov-report=term-missing -q >/dev/null 2>&1"; then
        failed_checks+=("Pytest tests")
    fi

    # Step 9: Security analysis (optional - don't fail on warnings)
    print_status "Running security analysis..."
    if bandit -r python_proptest/ -s B311,B110 -f json -o bandit-report.json >/dev/null 2>&1; then
