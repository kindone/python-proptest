...
def demo_public_api_access():
    ...
    for i in range(5):
        ...
        print(f"   Generated date: {month:2d}/{day:2d} (month {month} has {days_in_month(month)} days)")
    
    # 2. Fluent API - gen.chain()
    print("\n2. Fluent API Usage:")
    print("   base_gen.chain(lambda x: dependent_gen)")
    
    range_gen = Gen.int(5, 15).chain(
        lambda start: Gen.int(start, start + 10)
    )
    ...
    rect_gen = Gen.chain(
        Gen.int(1, 20),  # width
        lambda w: Gen.int(w, w + 10)  # height >= width
    )
    
    result = run_for_all(
        test_rectangle_area_property,
        rect_gen,
        num_runs=20
    )
