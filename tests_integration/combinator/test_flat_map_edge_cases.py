...
class TestFlatMapEdgeCases(unittest.TestCase):
    ...
    def test_nested_flatmap_independence(self):
        ...
        self.assertEqual(
            outer_values, outer_values2, "Outer values should be deterministic"
        )
        self.assertEqual(
            inner_values, inner_values2, "Inner values should be deterministic"
        )
    ...
    def test_flatmap_regeneration_determinism(self):
        ...
        if regenerated1 and regenerated2:
            ...
            for r1, r2 in zip(regenerated1, regenerated2):
                self.assertEqual(
                    r1, r2, f"Regenerated values should match: {r1} != {r2}"
                )
    ...
    def test_exception_safety_during_regeneration(self):
        ...
        try:
            ...
            self.assertIsNotNone(
                current_state, "RNG state should exist even after exception"
            )
    ...
    def test_flatmap_with_filter_chain(self):
        ...
        self.assertGreater(
            sum(shrinkable.value), 10, "Root should satisfy filter condition"
        )
        ...
        self.assertEqual(
            len(problems), 0, f"All shrinks should satisfy filter: {problems[:10]}"
        )
