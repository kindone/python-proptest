        ...
        for _ in range(10):
            ...
            def check_all_shrinks(sh, depth=0, max_depth=5):
                ...
                if sh.value < 5 or sh.value > 15:
                    problems.append((depth, sh.value, "out of [5, 15]"))
            ...
            self.assertEqual(
                len(problems), 0, f"Found values violating constraints: {problems[:5]}"
            )
        ...
        for _ in range(10):
            ...
            def check_all_shrinks(sh, depth=0, max_depth=5):
                ...
                if sh.value < 15 or sh.value > 25:
                    problems.append((depth, sh.value, "out of [15, 25]"))
                for child in sh.shrinks().to_list():
                    problems.extend(check_all_shrinks(child, depth + 1, max_depth))
                return problems
        ...
        for _ in range(10):
            ...
            def check_all_shrinks(sh, depth=0, max_depth=5):
                ...
                if sh.value < 9 or sh.value > 21:
                    problems.append((depth, sh.value, "out of [9, 21]"))
                for child in sh.shrinks().to_list():
                    problems.extend(check_all_shrinks(child, depth + 1, max_depth))
                return problems
        ...
        for _ in range(10):
            ...
            def check_all_shrinks(sh, depth=0, max_depth=5):
                ...
                if sh.value < 6 or sh.value > 12:
                    problems.append((depth, sh.value, "out of [6, 12]"))
                if sh.value % 3 != 0:
                    problems.append((depth, sh.value, "not divisible by 3"))
                for child in sh.shrinks().to_list():
                    problems.extend(check_all_shrinks(child, depth + 1, max_depth))
                return problems
        ...
        for _ in range(10):
            ...
            def check_all_shrinks(sh, depth=0, max_depth=5):
                ...
                if len(sh.value) < 3 or len(sh.value) > 8:
                    problems.append(
                        (depth, sh.value, f"length {len(sh.value)} out of [3, 8]")
                    )
                for child in sh.shrinks().to_list():
                    problems.extend(check_all_shrinks(child, depth + 1, max_depth))
                return problems
            ...
            def check_all_shrinks(sh, depth=0, max_depth=5):
                ...
                for elem in sh.value:
                    if elem < 10 or elem > 20:
                        problems.append(
                            (depth, sh.value, f"element {elem} out of [10, 20]")
                        )
                for child in sh.shrinks().to_list():
                    problems.extend(check_all_shrinks(child, depth + 1, max_depth))
                return problems
            ...
            def check_all_shrinks(sh, depth=0, max_depth=5):
                ...
                for elem in sh.value:
                    if elem < 15 or elem > 25:
                        problems.append(
                            (depth, sh.value, f"element {elem} out of [15, 25]")
                        )
                for child in sh.shrinks().to_list():
                    problems.extend(check_all_shrinks(child, depth + 1, max_depth))
                return problems
        ...
        for _ in range(10):
            ...
            def check_all_shrinks(sh, depth=0, max_depth=5):
                ...
                if sh.value < 8 or sh.value > 24:
                    problems.append((depth, sh.value, "out of [8, 24]"))
                if sh.value % 4 != 0:
                    problems.append((depth, sh.value, "not divisible by 4"))
                for child in sh.shrinks().to_list():
                    problems.extend(check_all_shrinks(child, depth + 1, max_depth))
                return problems
