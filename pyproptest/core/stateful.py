            ...
            try:
                ...
                for callback in self._cleanup_callbacks:
                    try:
                        ...
                        pass
                raise PropertyTestError(f"Stateful property failed on run {run + 1}: {e}")


def simpleActionGenOf(state_type: type, *action_gens: Generator[SimpleAction[S]]) -> Generator[SimpleAction[S]]:
...
def actionGenOf(state_type: type, model_type: type, *action_gens: Generator[Action[S, A]]) -> Generator[Action[S, A]]:
    """Create a generator that randomly selects from multiple action generators."""
    if not action_gens:
        raise ValueError("At least one action generator must be provided")
...
def statefulProperty(initial_state_gen: Generator[S], 
                    action_gen: Generator[Action[S, A]],
                    max_actions: int = 100,
                    num_runs: int = 100,
                    seed: Optional[Union[str, int]] = None,
                    initial_model_gen: Optional[Generator[A]] = None) -> StatefulProperty[S, A]:
    """Create a stateful property for testing."""
    return StatefulProperty(initial_state_gen, action_gen, max_actions, num_runs, seed, initial_model_gen)


def simpleStatefulProperty(initial_state_gen: Generator[S], 
                          action_gen: Generator[SimpleAction[S]],
                          max_actions: int = 100,
                          num_runs: int = 100,
                          seed: Optional[Union[str, int]] = None) -> StatefulProperty[S, A]:
