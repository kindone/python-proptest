...
from pyproptest import (
    Gen, SimpleAction, Action, StatefulProperty,
...
class TestStateful:
    ...
    def test_simple_stateful_property(self):
        ...
        pop_gen = Gen.just(
            SimpleAction(lambda obj: obj.pop() if obj else None)
        )
        
        clear_gen = Gen.just(
            SimpleAction(lambda obj: obj.clear() if obj else None)
        )
        
        # Create weighted action generator
        action_gen = simpleActionGenOf(list, push_gen, pop_gen, Gen.weighted_value(clear_gen, 0.1))
        
        # Create stateful property
        prop = simpleStatefulProperty(
            Gen.list(Gen.int(min_value=0, max_value=10000), min_length=0, max_length=20),
            action_gen,
            max_actions=50,
            num_runs=5,
            seed=42
    ...
    def test_stateful_property_with_model(self):
        ...
        increment_gen = Gen.just(
            Action(lambda state, model: setattr(state, 'value', state.value + 1) or setattr(model, 'value', model.value + 1))
        )
        
        decrement_gen = Gen.just(
            Action(lambda state, model: setattr(state, 'value', max(0, state.value - 1)) or setattr(model, 'value', max(0, model.value - 1)))
        )
        
        reset_gen = Gen.just(
            Action(lambda state, model: setattr(state, 'value', 0) or setattr(model, 'value', 0))
        )
        
        # Create action generator
        action_gen = actionGenOf(Counter, CounterModel, increment_gen, decrement_gen, reset_gen)
    ...
    def test_stateful_property_with_complex_state(self):
        ...
        set_data_gen = Gen.tuple(
            ...
            lambda t: SimpleAction(lambda state: setattr(state, 'data', {**state.data, t[0]: t[1]}) or state.history.append(f"set_{t[0]}"))
        )
        
        increment_counter_gen = Gen.just(
            SimpleAction(lambda state: setattr(state, 'counter', state.counter + 1) or state.history.append("increment"))
        )
        
        clear_data_gen = Gen.just(
            SimpleAction(lambda state: setattr(state, 'data', {}) or state.history.append("clear"))
        )
        
        action_gen = simpleActionGenOf(ComplexState, set_data_gen, increment_counter_gen, clear_data_gen)
        
        prop = simpleStatefulProperty(
            Gen.just(ComplexState()),
            action_gen,
            max_actions=15,
            num_runs=2,
            seed=999
    ...
    def test_stateful_property_with_different_seeds(self):
        ...
        # Simple action
        action_gen = Gen.just(
            SimpleAction(lambda obj: obj.append(random.randint(1, 100)) if obj is not None else None)
        )
    ...
    def test_stateful_property_with_zero_actions(self):
        ...
        action_gen = Gen.just(
            SimpleAction(lambda obj: None)  # No-op action
        )
