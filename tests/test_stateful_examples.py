...
import pytest
from pyproptest import simpleStatefulProperty, statefulProperty, Gen, SimpleAction, Action, simpleActionGenOf, actionGenOf
...
class TestStatefulExamples:
    ...
    def test_stack_operations(self):
        ...
        # Action: Pop element
        pop_action = Gen.just(SimpleAction(
            lambda stack: stack[:-1] if stack else stack
        ))
    ...
    def test_simple_stateful_with_model(self):
        ...
        # Action: Increment
        increment_action = Gen.just(Action(
            lambda counter, model: None  # Simple increment action
        ))
