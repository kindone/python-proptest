...
import pytest
from pyproptest import for_all, given, example, settings, assume, note
...
class TestDecoratorAPIExamples:
    ...
    def test_dictionary_properties(self):
        ...
        @for_all(dictionaries(text(min_size=1, max_size=5), integers(min_value=0, max_value=100)))
        ...
        @for_all(dictionaries(text(min_size=1, max_size=3), integers(), min_size=0, max_size=10))
    ...
    def test_assume_function(self):
        ...
        def test_division_property(x: int, y: int):
            ...
            assert result * y == x  # Division is inverse of multiplication
