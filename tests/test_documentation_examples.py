...
import json
import unittest
from python_proptest import run_for_all, for_all, Gen, Property, PropertyTestError
...
class TestGeneratorsDocumentationExamples(unittest.TestCase):
    ...
    def test_string_generators_examples(self):
        ...
        # Note: Using regular str generator with printable charset
        result = run_for_all(
            lambda s: isinstance(s, str) and 0 <= len(s) <= 5,
            Gen.str(min_length=0, max_length=5, charset="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789")
        )
        assert result is True
    ...
    def test_list_generator_examples(self):
        ...
        result = run_for_all(
            lambda lst: isinstance(lst, list) and 2 <= len(lst) <= 5 and all(isinstance(x, bool) for x in lst),
        ...
        result = run_for_all(
            lambda lst: isinstance(lst, list) and 0 <= len(lst) <= 10 and all(isinstance(x, str) and 1 <= len(x) <= 3 for x in lst),
    ...
    def test_dict_generator_example(self):
        ...
        result = run_for_all(
            lambda d: (isinstance(d, dict) and 1 <= len(d) <= 3 and 
                      all(isinstance(k, str) and len(k) == 1 and 'a' <= k <= 'z' for k in d.keys()) and
    ...
    def test_tuple_generator_examples(self):
        ...
        result = run_for_all(
            lambda t: (isinstance(t, tuple) and len(t) == 2 and 
                      isinstance(t[0], bool) and isinstance(t[1], float)),
        ...
        result = run_for_all(
            lambda t: (isinstance(t, tuple) and len(t) == 3 and 
                      isinstance(t[0], str) and isinstance(t[1], int) and isinstance(t[2], str)),
...
class TestCombinatorsDocumentationExamples(unittest.TestCase):
    ...
    def test_map_combinator_examples(self):
        ...
        result = run_for_all(
            lambda obj: (isinstance(obj, dict) and 
                        "id" in obj and "email" in obj and
                        isinstance(obj["id"], int) and 1 <= obj["id"] <= 100 and
                        obj["email"] == f"user{obj['id']}@example.com"),
            user_object_gen
        )
        assert result is True
    ...
    def test_flat_map_combinator_examples(self):
        ...
        string_gen = Gen.int(min_value=1, max_value=5).flat_map(lambda n: Gen.str(min_length=n, max_length=n))
        
        result = run_for_all(
            lambda s: isinstance(s, str) and 1 <= len(s) <= 5,
            string_gen
        )
        assert result is True
    ...
    def test_one_of_combinator_examples(self):
        ...
        # Union of ranges
        union_gen = Gen.one_of(Gen.int(min_value=0, max_value=10), Gen.int(min_value=20, max_value=30))
    ...
    def test_construct_combinator_examples(self):
        ...
        result = run_for_all(
            lambda p: isinstance(p, Point) and isinstance(p.x, int) and isinstance(p.y, int),
            point_gen
        )
        assert result is True
...
class TestPropertiesDocumentationExamples(unittest.TestCase):
    ...
    def test_property_class_example(self):
        ...
        prop.for_all(Gen.int(min_value=1, max_value=100), Gen.int(min_value=1, max_value=100))
...
class TestShrinkingDocumentationExamples(unittest.TestCase):
    ...
    def test_integer_shrinking_example(self):
        ...
        try:
            ...
            self.assertGreater(minimal_value, 10)  # Should be the smallest failing value
    ...
    def test_string_shrinking_example(self):
        ...
        try:
            ...
            self.assertGreater(len(minimal_string), 3)  # Should be the shortest failing string
...
class TestStatefulTestingDocumentationExamples(unittest.TestCase):
    ...
    def test_simple_stateful_property_example(self):
        ...
        from python_proptest import simpleStatefulProperty, SimpleAction, simpleActionGenOf
        ...
        add_action_gen = Gen.int().map(lambda val:
            SimpleAction(lambda arr: arr.append(val))
        )

        # Action: Clear the list
        clear_action_gen = Gen.just(
    ...
    def test_stateful_property_with_model_example(self):
        ...
        from python_proptest import statefulProperty, Action, actionGenOf
        ...
        # Action: Add an element
        add_action_gen = Gen.int().map(lambda val:
            Action(lambda arr, model: arr.append(val))
        )

        # Action: Clear the list
        clear_action_gen = Gen.just(
            Action(lambda arr, model: arr.clear())
        )
        ...
        # Create the stateful property
        prop = statefulProperty(initial_gen, action_gen, initial_model_gen=initial_model_gen)
