...
import pytest
from pyproptest import for_all, given, example, settings, assume, note, run_property_test
from pyproptest import integers, floats, text, lists, dictionaries, one_of, just, Gen
...
class TestDecoratorAPI:
    ...
    def test_decorator_with_strategies(self):
        ...
        @for_all(integers(min_value=1, max_value=100), integers(min_value=1, max_value=100), integers(min_value=1, max_value=100))
    ...
    def test_decorator_with_dictionaries(self):
        ...
        @for_all(dictionaries(text(min_size=1, max_size=5), integers(), min_size=0, max_size=5))
    ...
    def test_decorator_with_nested_structures(self):
        ...
        @for_all(lists(dictionaries(text(min_size=1), integers(), min_size=1, max_size=3), min_size=0, max_size=5))
    ...
    @for_all(integers(min_value=1, max_value=100).filter(lambda x: x % 2 == 0).map(lambda x: x * 2))
