...
class TestFlatMapRNGPreservation(unittest.TestCase):
    ...
    def test_flat_map_regeneration_uses_correct_rng_state(self):
        ...
        second_gen = Gen.list(
            Gen.int(1, 10), min_length=first_value, max_length=first_value
        )
        ...
        if len(first_shrinks) > 0:
            ...
            wrong_regenerated_gen = Gen.list(
                Gen.int(1, 10),
                min_length=shrunk_first.value,
                max_length=shrunk_first.value,
            )
            ...
            correct_regenerated_gen = Gen.list(
                Gen.int(1, 10),
                min_length=shrunk_first.value,
                max_length=shrunk_first.value,
            )
            ...
            for shrink in flat_map_shrinks:
                if (
                    isinstance(shrink.value, list)
                    and len(shrink.value) == shrunk_first.value
                ):
            ...
            if matching_shrink:
                ...
                self.assertEqual(
                    matching_shrink.value,
                    correct_regenerated.value,
                    "flat_map regeneration should match correct RNG state restoration",
                )
                self.assertNotEqual(
                    matching_shrink.value,
                    wrong_regenerated.value,
        ...
        def serialize_tree(shrinkable, max_depth=3):
            ...
            def serialize_recursive(sh, depth=0):
                ...
                result = {
                    ...
                    "value_len": (
                        len(sh.value)
                        if isinstance(sh.value, (list, tuple, str))
                        else None
                    ),
                    "shrinks": [],
                }
