...
import pytest
from pyproptest import Gen, for_all, run_for_all, integers, text, lists, dictionaries, one_of, just, floats
...
class TestIntGenerator:
    ...
    def test_int_generator_negative_range(self):
        ...
        result = run_for_all(
            lambda x: -50 <= x <= 50,
            Gen.int(min_value=-50, max_value=50),
            num_runs=50
        )
        assert result is True
...
class TestDictGenerator:
    ...
    def test_dict_generator_basic(self):
        ...
        result = run_for_all(
            ...
            Gen.dict(Gen.str(min_length=1, max_length=3), Gen.int(min_value=0, max_value=10)),
            num_runs=50
        )
        assert result is True
    ...
    def test_dict_generator_size(self):
        ...
        result = run_for_all(
            lambda d: 0 <= len(d) <= 3,
            Gen.dict(Gen.str(min_length=1, max_length=2), Gen.int(min_value=0, max_value=5), min_size=0, max_size=3),
            num_runs=50
        )
        assert result is True
