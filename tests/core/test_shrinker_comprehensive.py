...
def compare_shrinkable(lhs: Shrinkable, rhs: Shrinkable, max_elements: int = 1000) -> bool:
...
class TestShrinkerComprehensive(unittest.TestCase):
    ...
    def test_shrinkable_concat(self):
        ...
        assert serialize_shrinkable(shr0) == '{"value":2,"shrinks":[{"value":0},{"value":1}]}'
        shr1 = shr0.concat(lambda parent: Stream.one(Shrinkable(parent.value + 5)))
    ...
    def test_shrinkable_retrieve(self):
        ...
        assert serialize_shrinkable(shr.retrieve([0])) == serialize_shrinkable(Shrinkable(0))
        assert serialize_shrinkable(shr.retrieve([1])) == '{"value":2,"shrinks":[{"value":1}]}'
    ...
    def test_shrinkable_float_shrinks_values_with_exponent_0_correctly(self):
        ...
        for value in test_cases:
            ...
            if len(shrinks_list) > 1:
                second_shrink = shrinks_list[1]
                assert second_shrink.value < value, f"{value} should shrink to a smaller value"
                assert second_shrink.value >= 0.0
        ...
        if len(shrinks_list) > 1:
            ...
            if not second_shrinks.is_empty():
                ...
                if len(second_shrinks_list) > 1:
                    ...
                    assert third_shrink.value != second_shrink.value, \
