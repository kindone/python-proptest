...
class TestUnicodeStringGenerator(unittest.TestCase):
    ...
    def test_unicode_string_generator_shrinking(self):
        ...
        # When min_length > 0, we can only shrink to shorter strings if current length > min_length
        # If current length == min_length, we can only shrink elements (codepoints), not length
        if len(shrinkable.value) > 0:
